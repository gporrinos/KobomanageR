...
#' \itemize{
#'   \item `kobo_projects()` to retrieve asset metadata when `name` is used
#'   \item `kobotools_messages()` for multilingual messages
...
kobo_download <- function(url,
  ...
  msg = kobotools_messages(language)
  ...
  tryCatch(
    {
      ...
      downloaded_from <-
        if(!is.null(name)) paste0(" ",msg$from_form, " '", name, "'") else ""
      message(paste0("      ",
...
#' \itemize{
#'   \item `kobo_projects()` to retrieve asset metadata
#'   \item `kobo_download()` to fetch submissions
#'   \item `kobotools_messages()` for multilingual messages
#'   \item `coalesce_to()` for safe value extraction
#' }
#'
#' @export
kobo_delete_submission <- function(url     = "eu.kobotoolbox.org",
  ...
  # --- Obtain 'assetid' if not provided --- #
  if(is.null(assetid)){
    api     = kobo_projects(url = url, username = username, password = password)
    assetid = api$assetid[which(api$name == name)]
    if (length(assetid) == 0) {
  ...
  if(is.null(submission_id)){
    ...
    ids   = unlist( lapply( dat, function(x) coalesce_to(x$'_id') ) )
    uuids = unlist( lapply( dat, function(x) coalesce_to(x$'_uuid') ) )
